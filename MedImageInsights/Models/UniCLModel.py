...
from MedImageInsights.ImageEncoder import build_image_encoder
from MedImageInsights.LangEncoder import build_lang_encoder
from MedImageInsights.LangEncoder import build_tokenizer
...
def load_pretrained_model(model, repo_id: str="joortif/medimageinsights", filename: str="medimageinsight-v1.0.0.pt", pretrained_layers=['*'], verbose=True):
    logger.info('=> Model not found in local directory, downloading MedImageInsight pretrained model from HuggingFace')
    local_path = hf_hub_download(
        repo_id=repo_id,
        filename=filename,
        cache_dir=pathlib.Path.home() / ".cache" / "medimageinsights"
    )

    model.from_pretrained(local_path, pretrained_layers, verbose)
...
def build_unicl_model(config, **kwargs):
    ...
    if config['UNICL_MODEL']['LOAD_PRETRAINED']:
        ...
        from MedImageInsights.Distributed.Utils import is_valid_url, download_file
